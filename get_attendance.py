...
get_fsize = f"du -b {file_path} | cut -f 1" # ファイルのサイズ(B)を取得するShell Script
...
async def get_attendance():
    global p # 前回のファイルサイズ
    c = ssh.exec_command(get_fsize)[1].readline() # 現在のファイルサイズ
    if p < c: # ファイルサイズが増加した場合
        # 学籍番号と日時の情報を discord で発言
        await channel.send(ssh.exec_command(f'cat {file_path} | tail -n 4 | tr "\n" " " | cut -b 19-27,50-70')[1].readline())
    p = c

try:
    p = ssh.exec_command(get_fsize)[1].readline() # 前回のファイルサイズ
